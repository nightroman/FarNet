...
# empty package is invalid for NuGet
task InvalidPackage {
	$1 = [System.IO.Packaging.Package]::Open("$PSScriptRoot\z.nupkg", 'Create', 'ReadWrite')
	$1.Close()

	try { throw Restore-FarPackage z.nupkg }
	catch {"$_"; assert ("$_" -match '^Package must have one \*\.nuspec:')}
