...
namespace FarNet.Works
{
	public sealed class ProxyTool : ProxyAction, IModuleTool
	{
		...
		{
			using (Log log = Log.Switch.TraceInfo ? new Log("Invoking {0} From='{1}'", (_Handler != null ? Log.Format(_Handler.Method) : ClassName), e.From) : null)
			{
				Invoking();
		...
		{
			return Invariant.Format("{0} Options='{1}'", base.ToString(), Attribute.Options);
		}
