...
module ModalDialogEditorIssues =
    ...
    let test = async {
        ...
        // nasty Far message
        do! wait (fun _ -> isDialog () && dt 1 = "It is impossible to edit the folder")
        do! Job.keys "Esc"
...
module MacroInvalid =
    ...
    let test = async {
        Async.farStart flow
        // our async exception
        do! wait (fun _ -> isDialog () && dt 0 = "ArgumentException" && dt 3 = "Macro: bar" && dt 4 = "Parameter name: macro")
        do! Job.keys "Esc"
        // done
        do! wait isFarPanel
    }
...
async {
    do! Job.func (fun _ -> if far.Window.Count <> 2 then failwith "Close all but panels.")
