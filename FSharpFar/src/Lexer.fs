            ...
            let rec tryFindStartColumn tokens =
               ...
               | {Kind = Ident; Token = t1} :: {Kind = SymbolKind.Other; Token = t2} :: remainingTokens ->
                    if t2.Tag = FSharpTokenTag.DOT then
                        //[rk] case: x.ToString().C // t1 ~ C ; t2 ~ .
                        // If the next is Ident then continue search.
                        // Else return the found so far or we will get None.
                        match remainingTokens with
                        | {Kind = SymbolKind.Ident} :: _ ->
                            tryFindStartColumn remainingTokens
                        | _ ->
                            Some t1.LeftColumn
                    else
                        Some t1.LeftColumn
               //[rk] case: n1.``n2 // t1 ~ n2 ; t2 ~ ` ; t3 ~ ` ; t4 ~ .
               // Each backtick is represented by "Keyword" named "RESERVED" with an unknown tag.
               | {Kind = Ident} :: {Kind = Keyword; Token = t2} :: {Kind = Keyword; Token = t3} :: {Kind = SymbolKind.Other; Token = t4} :: remainingTokens when
                    t2.FullMatchedLength = 1 && t3.FullMatchedLength = 1 && t4.Tag = FSharpTokenTag.DOT ->
                        tryFindStartColumn remainingTokens
