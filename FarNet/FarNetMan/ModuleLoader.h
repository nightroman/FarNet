...
public:
	...
	static void UnloadEntry(BaseModuleEntry^ entry);
...
private:
	...
	static void WriteCache(ModuleManager^ manager, List<ModuleCommandInfo^>^ commands, List<ModuleEditorInfo^>^ editors, List<ModuleFilerInfo^>^ filers, List<ModuleToolInfo^>^ tools);
private:
	// Static
	ModuleLoader() {}
	// Registered modules
