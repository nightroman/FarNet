...
Provider=Microsoft.Jet.OLEDB.4.0;Data Source=$([environment]::CurrentDirectory);Extended Properties="text;hdr=yes;fmt=delimited"
"@
try {
	$DbConnection.Open()
}
...
# get data now
Get-ChildItem $ENV:WINDIR | Select-Object Name,Length | Export-Csv test1.csv -NoTypeInformation
Get-ChildItem $ENV:WINDIR\SYSTEM32 | Select-Object Name,Length | Export-Csv test2.csv -NoTypeInformation
