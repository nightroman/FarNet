...
.Description
	Requires pandoc.exe and HtmlToFarHelp.exe
	...
	$env:Markdown
	Specifies the fixed pandoc format.
	Default: .text files ~ `markdown_phpextra`, others ~ `gfm`.

	$env:MarkdownBrowser
	Specifies the custom browser for the result HTML. E.g. if the default is
	Chrome then it cannot open local URLs like "file://...htm#current-topic".
	But Firefox can.

	$env:MarkdownCss
	Specifies the custom CSS file or URI for the result HTML.
	This file is good for GitHub style: https://gist.github.com/dashed/6714393
