...
$Data.File = "$PSScriptRoot\Open-TextLink.fas.ps1"
...
job {
	...
	$r = Get-TextLink.ps1
	$env:TextLinkEnv = $old
	Assert-Far $r -eq "%FarNetCode%\Test\Utility\Open-TextLink.fas.ps1(10): Text1"
}
keys Esc
job {
	# VS line link with %var%
	Open-TextLink.ps1 "noise %FarNetCode%\Test\Utility\Open-TextLink.fas.ps1(11) noise"
}
job {
...
job {
	...
	Open-TextLink.ps1 "noise %FarNetCode%\Test\Utility\Open-TextLink.fas.ps1:11"
}
job {
	Assert-Far (Get-TextLink.ps1) -eq "$($Data.File)(11): Text2"
}
