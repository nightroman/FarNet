...
job {
	# open editor
	Open-FarEditor "$env:FarHome\Far.exe.config"
}
job {
	# IsModal
	Assert-Far (!$Far.Window.IsModal)
	...
	Assert-Far $name1 -eq $name2
	Assert-Far $name1 -eq "$env:FarHome\Far.exe.config"
}
...
job {
	...
	Assert-Far $Far.Window.GetNameAt(-1) -eq "$env:FarHome\Far.exe.config"
}
