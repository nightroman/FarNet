	...
	class XPathObjectNode
	{
		...
		public bool HasChildren
		{
			get
			{
				...
				IList<XPathObjectNode> elements = null;
				while (elements == null)
				{
					elements = (IList<XPathObjectNode>)_elements.Target;
					if (elements != null)
						break;
					ActivateElements();
					elements = (IList<XPathObjectNode>)_elements.Target;
		...
		void ActivateAttributes() //?? lock was used, why?
		{
			...
			if (_target is ValueType || _target is string) //rvk "Linear types". Perhaps we need more.
			{
				...
				_elements = new WeakReference(_emptyElements);
			}
		...
		void ActivateElements() //?? lock was used, why?
		{
			...
			if (_elements != null && _elements.Target != null) //???
				return;
		...
		void ActivateDictionary()
		{
			...
			foreach (DictionaryEntry entry in (IDictionary)_target)
			{
				...
				var node = new XPathObjectNode(_context, entry.Value, null, this, elements.Count); //???
			...
			if (elements.Count == 0)
				...
				_elements = new WeakReference(elements);
		}
		void ActivateCollection()
		{
			...
			foreach (object val in (ICollection)_target)
			{
				...
				elements.Add(new XPathObjectNode(_context, val, null, this, elements.Count));
			}
		...
		void ActivateSimple()
		{
			...
			if (info != null)
			{
				...
				_elements = new WeakReference(_emptyElements); //??? elements
			}
			else
			{
				...
				foreach (PropertyInfo pi in _target.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
				{
					...
					if (str != null)
						...
						elements.Add(new XPathObjectNode(_context, val, pi.Name, this, elements.Count));
				}
		...
		void ActivateSuperFileElements()
		{
			...
			if (explorer != null)
			{
				...
				foreach (var it in explorer.GetFiles(argsFiles))
				{
					...
					// add
					elements.Add(new XPathObjectNode(_context, new SuperFile(explorer, it), null, this, elements.Count));
				}
			}
